...
public abstract class AudGameWindow extends JFrame {
	...
	public AudGameWindow(int timerInterval) {
		...
		// Game Timer
		gameTimer = new Timer(timerInterval, new ActionListener() {
			@Override
		...
		// Keyboard
		keyTimer = new Timer(KEY_TIMER_INTERVAL, new ActionListener() {
			@Override
	...
	/**
	 * Return the width of the game area (which may be different from the windows
	 * width).
	 */
	...
	/**
	 * Return the height of the game area (which may be different from the windows
	 * height).
	 */
	...
	/**
	 * Repaint the game. This is called by the game timer after updateGame. The
	 * method implements double buffering around paintGame, which is the method you
	 * should override to paint the game.
	 */
	private void repaintGame() {
		...
		if (now - fpsTime > 1000) {
			fps = String.format("%.1f fps", (1000.0 * fpsCount) / (now - fpsTime));
			fpsTime = now;
			fpsCount = 0;
		}
		...
		do {
			do {
				...
				Graphics2D g2 = (Graphics2D) strategy.getDrawGraphics().create(pane.getX(), pane.getY(),
	...
	/**
	 * Set the keys that repeat their handleInput calls when being held down. For
	 * example, in order to make the left and right keys repeatable, use
	 * setRepeatableKeys(KeyEvent.VK_LEFT, KeyEvent.VK_RIGHT).
	 * 
	 * Calling this method overwrites existing repeatable key settings.
	 * 
	 * @param keyCodes The repeatable keys. Possible values are the key codes
	 *                 defined in java.awt.event.KeyEvent (e.g. KeyEvent.VK_SPACE).
	 *                 Note that this is a "varargs" parameter, i.e. any number of
	 *                 key code parameters can be specified.
	 */
	 ...
	 * 
	 * @return the system time in milliseconds at which the start() method was
	 *         called
	 */
	...
	/**
	 * Update the game, move objects etc. This is called by the game timer once
	 * every timerInterval milliseconds (this value can changed in the constructor
	 * and defaults to 32ms).
	 * 
	...
	/**
	 * Paint the game. This is called automatically when the game needs to be
	 * painted.
	 * 
	 ...
	 * 
	 * @param keyCode The pressed key. The value is the one of the key codes defined
	 *                in java.awt.event.KeyEvent (e.g. KeyEvent.VK_SPACE).
	 */
